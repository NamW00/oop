...
#include <ctime> // 랜덤 함수를 사용하기 위한 헤더 파일
...
#pragma region code12(양의 정수 N보다 작거나 같은 수중 가장 큰 2의 거듭제곱 수 출력)
	...
	*/
#pragma endregion
#pragma region code13(1사분면에서 원점으로 부터의 거리가 100이하인 정수 좌표점)
		// 중첩된 for문 사용 예제
	/*	
		int count = 0;
		double distance = 0.0;
	...
	*/
#pragma endregion
#pragma region code14(1사분면에서 y=x의 위쪽에 있으면서 원점과 거리가 100이하인 정수 좌표)
	/*	
		int count = 0;
		for (int x = 0; x <= 100; x++) {
			for (int y = 0; y <= 100; y++) {
				if (x <= y && x * x + y * y <= 10000)
					count++;
			}
		}
		cout << "The count is " << count << endl;
	...
	*/
#pragma endregion
#pragma region code15(2에서 N사이의 정수들 중에서 서로 약수-배수관계의 정수쌍 개수)
	/*	
		// 단 (a, b)와 (b, a)는 같은 쌍으로 간주, (a, a)는 카운트 하지 않는다.
		// 2 ~ 10 사이의 약수-배수 관계 정수쌍
		...
		for (int a = 2; a <= N; ++a)	// a > b일때 가정
		{
			for (int b = 2; b <= a/2; ++b) // a의 약수가 a/2보다 클수는 없으므로 이렇게도 범위를 줄일 수 있다.
			{
				if (a%b==0)
				{
					count++;
	...
	*/
#pragma endregion
#pragma region code16(Gambler s Ruin Simula10tion)
	/*	
		// 동전을 던져서 앞면이 나오면 +1, 뒷면이 나오면 -1
		...
		// pseudo-random number generater
		// 가짜 랜덤 수 생성기
		// 현재 시각을 나타내는 time함수를 이용하여
		// srand 내부에 넣을 정수를 time으로 대체하는 느낌이다.
	
		cout << "초기자본(stake), 목표액(goal), 학습 횟수(T) 입력: ";
		cin >> stake >> goal >> T;
